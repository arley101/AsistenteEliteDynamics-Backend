...
try:
    ...
    # raise ImportError(f"No se pudo importar 'hacer_llamada_api' o constantes: {e}") from e # Comentado para permitir carga
...
def _get_azure_mgmt_token(parametros: Dict[str, Any]) -> str:
    ...
    try:
        ...
        token_info = _pa_credential_instance.get_token(AZURE_MGMT_DEFAULT_SCOPE[0])
...
def _get_arm_auth_headers(parametros_auth: Dict[str, Any]) -> Dict[str, str]:
    ...
    try:
        token = _get_azure_mgmt_token(parametros_auth)
...
def listar_flows(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        arm_headers = _get_arm_auth_headers(parametros)
...
def obtener_flow(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not nombre_flow:
        return {"status": "error", "message": "'nombre_flow' es requerido."}
    ...
    try:
        ...
        return {"status": "error", "message": "Fallo de autenticación ARM.", "details": str(auth_err)}
    ...
    try:
        ...
        if isinstance(e, requests.exceptions.HTTPError) and e.response is not None:
            ...
            try:
                ...
                details = e.response.text
            if status_code == 404:
                return {"status": "error", "message": f"Flow '{nombre_flow}' no encontrado.", "details": details, "http_status": 404} # Añadido http_status
...
def ejecutar_flow(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        response = requests.post(flow_trigger_url,
                                 headers=request_headers,
                                 json=payload if payload and request_headers.get('Content-Type') == 'application/json' else None,
                                 data=payload if payload and request_headers.get('Content-Type') != 'application/json' else None,
                                 timeout=max(AZURE_MGMT_API_DEFAULT_TIMEOUT, 120))
