...
def crear_documento_word(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # Alternativamente, se puede crear un item con 'file': {} y luego subir contenido,
    # pero para un archivo vacío, subir 0 bytes a /content es más directo.
...
def obtener_documento_word_binario(parametros: Dict[str, Any], headers: Dict[str, str]) -> Union[bytes, Dict[str, Any]]:
    ...
    # Esta función es idéntica a onedrive_actions.descargar_archivo, solo cambia el log.
    # Podríamos llamar a esa función o duplicar la lógica. Por claridad, duplicamos con logs específicos.
    item_id_o_ruta: Optional[str] = parametros.get("item_id_o_ruta")
    if not item_id_o_ruta:
        return {"status": "error", "message": "Parámetro 'item_id_o_ruta' es requerido."}

    if "/" in item_id_o_ruta: url = f"{BASE_URL}/me/drive/root:{item_id_o_ruta.strip('/')}:/content"
    else: url = f"{BASE_URL}/me/drive/items/{item_id_o_ruta}/content"
    ...
    try:
        ...
        if isinstance(e, requests.exceptions.HTTPError) and e.response is not None:
            ...
            if status_code == 404: return {"status": "error", "message": f"Documento Word '{item_id_o_ruta}' no encontrado.", "details": details}
...
def crear_libro_excel(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # Esta función es muy similar a crear_documento_word, solo cambia el Content-Type y extensión.
    nombre_archivo: Optional[str] = parametros.get("nombre_archivo")
    ruta_onedrive: str = parametros.get("ruta_onedrive", "/")
    conflict_behavior: str = parametros.get("conflict_behavior", "rename")
...
def leer_celda_excel(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # Para archivos en SharePoint Drive: /drives/{drive-id}/items/{item-id}/workbook/...
    # Para OneDrive: /me/drive/items/{item-id}/workbook/...
    ...
    # Usar comillas simples alrededor de la dirección en range() o usedRange()
    # El endpoint es sensible a si la hoja se referencia por ID o nombre.
    # Si es nombre, se puede usar /workbook/worksheets('NombreHoja')/range...
    # Si es ID, se usa /workbook/worksheets/{id}/range...
    # Por simplicidad, asumimos que el usuario provee el nombre o ID correcto para la URL.
    url = f"{drive_prefix}/items/{item_id}/workbook/worksheets/{hoja_nombre_o_id}/range(address='{celda_o_rango_direccion}')"
    # Seleccionar campos útiles: text (lo que se ve), values (datos crudos), formulas, address
...
def escribir_celda_excel(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # 'valores' debe ser una lista de listas, ej: [[valor_A1, valor_B1], [valor_A2, valor_B2]]
    # Para una sola celda: [[valor]]
    ...
    try:
        # PATCH a un rango actualiza sus valores. Devuelve el objeto de rango actualizado.
...
def crear_tabla_excel(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # La dirección en el body es la dirección completa del rango, incluyendo nombre de hoja.
    body: Dict[str, Any] = {"address": f"{hoja_nombre_o_id}!{rango_direccion}", "hasHeaders": tiene_headers_tabla}
    if nombre_tabla: body["name"] = nombre_tabla # Opcional
...
def agregar_filas_tabla_excel(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # 'valores_filas' debe ser una lista de listas, donde cada sublista es una fila.
    # El número de elementos en cada sublista debe coincidir con el número de columnas de la tabla.
    ...
    try:
        # POST a /rows devuelve información sobre las filas añadidas (ej. el índice)
